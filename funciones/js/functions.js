...
function findLongestWord( array_words ){
	var longest = array_words[0].length;
	for (var index = 1; index < array_words.length; index ++) {
		if (array_words[index].length > longest) {
			longest = array_words[index].length;
		}
	}
	return longest;
}
...
array_lengths.push( findLongestWord( ["Conditional", "Unconditionally", "Unconditional"] ) );
array_lengths.push( findLongestWord( ["Slovania", "Croatia", "Bosnia and Herzegovina"] ) );
...
function charFreq( string ) {
	...
	while (index < string.length) {
		...
		if (letter in freqCounter === false) {
...
function beautifyLetters( array )
{
	...
	for (var index = 0; index < array.length; index++) {
		if (typeof array[index] === "string") {
			item = array[index].toLowerCase();
			if (isVowel.indexOf(item) != -1) {
	...
	for (var index = 0; index < array.length; index++) {
		if (typeof array[index] === "number" && array[index] >= 10) {
			while (array[index] >= 10) {
				var bigNum = array[index].toString().split('');
				var newNum = 0; 
				for (var i = 0; i < bigNum.length; i++) {
					newNum += parseInt(bigNum[i]);
				}
				array[index] = newNum;
			}
		}
...
function sortArray( array )
{
	...
	for (index = 0; index < array.length; index ++) {
		if(typeof array[index] === "number") {
			arrayNum.push(array[index]);
		}
		else {
			arrayStr.push(array[index]);
		}
	}
	arrayNum.sort();
	...
	return arrayNum.concat(arrayStr);
}
